...
| Current published version | `1.14.47` (2026-09-15) |
...
OPENCODE_VERSION=1.14.47 bun run build -- --single

# Full multi-platform build (all 12 targets, cross-compile) — for npm publish:
cd packages/opencode
OPENCODE_VERSION=1.14.47 bun run build
```
...
Verify after build: `opencode --version` must print `1.14.47` (or the current fork release version). If it prints `0.0.0-master-...`, the build forgot `OPENCODE_VERSION` — rebuild it.
...
**Consequence**: building without `OPENCODE_VERSION=1.14.47` produces `0.0.0-master-{date}`, which:
...
**Rule: every build MUST set `OPENCODE_VERSION` to the current fork release version (`1.14.47`).** No exceptions.
...
| migration-marker-db-path | `src/index.ts` (CLI yargs middleware, ~line 116) | JSON→sqlite migration marker must be the **actual DB file** (`Database.Path` from `src/storage/db.ts`, which honors `OPENCODE_DB` incl. `:memory:`/absolute/relative names plus channel naming `opencode-{channel}.db`). The old hardcoded `path.join(Global.Path.data, "opencode.db")` never matched for custom/channel DBs → migration re-ran on every startup (slow, noisy, could resurrect rows deleted from sqlite). Guard skips migration when `Path === ":memory:"`. Verify after any upstream merge that `const marker = Database.Path` is intact. | issue #21 |
| publish-race-probe-retry | `script/publish.ts` (`published()`, failure path of `publish()`) | After a successful PUT, npm keeps the version in a "being processed" window during which `npm view pkg@ver` still 404s. A second racing publisher (double CI dispatch / manual vs CI) then mis-reads the probe as "not published", retries the PUT, gets the conflict, and fails the whole run even though everything published fine (observed 2026-09-15 during v1.14.47). Fix: treat npm's conflict signature `"cannot publish over the previously published versions"` as definitive success, and retry the `npm view` probe with backoff (`published(name, version, 3)`). Preserve both across upstream syncs — this file is fork-specific (§4.5). | v1.14.47 release incident |
...
2. If CLI behavior changed: rebuild with `OPENCODE_VERSION=1.14.47 bun run build -- --single` and smoke-test `opencode --version`.
